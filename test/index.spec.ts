...
const testHandler =
  (test: string, results: string[]) =>
  async ({ data }, _ctx, next) => {
...
describe("NatsRun", () => {
  ...
  describe("Route Matches", () => {
    ...
    it("matches a simple one part route", () => {
      ...
      assert.strictEqual(matches[0]({ subject: "order", data: toUint8Array("test") }), "test order test", 'Pattern match should have "order"');
    });
    ...
    it("matches a simple two part route", () => {
      ...
      assert(matches.length == 1, "Router pattern should have a match");
      assert(matches[0].length == 1, "Patter match should have a handler");
      assert.strictEqual(matches[0]({ subject: "order", data: toUint8Array("test") }), "test order.create test", 'Pattern match should have "order.create"');
    });
    ...
    it("matches a wildcard route", () => {
      ...
      assert(matches.length == 1, "Router pattern should have a match");
      assert(matches[0].length == 1, "Patter match should have a handler");
      assert.strictEqual(matches[0]({ subject: "order", data: toUint8Array("test") }), "test order.*.new test", 'Pattern match should have "order.*.new"');
    });
  ...
  describe("Route handler ordering", async () => {
    ...
    describe("ordering by specificity", () => {
      ...
      it("executes wildcard handlers in correct order", async () => {
        ...
        assert.deepStrictEqual(results, ["order.*.new test", "order.> test"], "Wildcard handlers should execute in correct order");
      });

      it("executes exact match handlers first", async () => {
        await router.handle("order.create", toUint8Array("test"));

        assert.deepStrictEqual(results, ["order.create test", "order.> test"], "Exact match handlers should execute before wildcards");
      });
    });
    ...
    describe("ordering by insertion", () => {
      ...
      it("executes handlers in order of insertion", async () => {
        ...
        assert.deepStrictEqual(results, ["order.> test", "order.*.new test", "order.create.new test"], "Handlers should execute in order of insertion");
      });

      it("maintains insertion order for wildcard matches", async () => {
        await router.handle("order.other.new", toUint8Array("test"));

        assert.deepStrictEqual(results, ["order.> test", "order.*.new test"], "Wildcard handlers should execute in order of insertion");
      });
    });
  ...
  describe("Error handling", () => {
    ...
    it("should handle invalid subject patterns", () => {
      assert.throws(() => router.add("", async (_) => {}), { message: /Invalid subject pattern/ }, "Should throw on empty subject");
    ...
    it("should handle handler errors gracefully", async () => {
      ...
      await assert.rejects(() => router.handle("order.create", toUint8Array("test")), error, "Should propagate handler errors");
    });
    ...
    it("should handle multiple handler errors", async () => {
      ...
      await assert.rejects(() => router.handle("order.create", toUint8Array("test")), error1, "Should propagate first handler error");
    });

    it("should handle invalid handler types", () => {
      assert.throws(() => router.add("order.create", null as any), { message: /Invalid handler/ }, "Should throw on null handler");

      assert.throws(() => router.add("order.create", undefined as any), { message: /Invalid handler/ }, "Should throw on undefined handler");

      assert.throws(() => router.add("order.create", "not a function" as any), { message: /Invalid handler/ }, "Should throw on non-function handler");
    });
    ...
    it("should handle invalid message types", async () => {
      ...
      await assert.doesNotReject(() => router.handle("order.create", undefined as any), "Should handle undefined message");
    });
  });
  ...
  describe("Handler Registration and Matching", () => {
    ...
    describe("Multiple Handlers", () => {
      it("executes multiple handlers for the same pattern", async () => {
        router.add("test.multi", [testHandler("handler1", results), testHandler("handler2", results)]);
  ...
  describe("Resource Management", () => {
    ...
    it("shows linear or better memory growth with increasing patterns", async () => {
      ...
      DEBUG && console.log("Memory measurements:", measurements);
      ...
      DEBUG && console.log("Growth ratios:", growthRatios);
      ...
      DEBUG && console.log(`Passed: ${growth.filter(Boolean).length}/${growthRatios.length}`);
      assert(
        growth.filter(Boolean).length > growthRatios.length / 2,
    ...
    it("maintains reasonable memory per pattern", async () => {
      ...
      const threshold = 3072; // 2KB per pattern as an example threshold
      ...
      DEBUG && console.log(`Average memory per pattern: ${memoryPerPattern.toFixed(2)} bytes`);

      assert(memoryPerPattern < threshold, `Memory per pattern (${memoryPerPattern.toFixed(2)} bytes) exceeds reasonable threshold (${threshold} bytes)`);
    });
